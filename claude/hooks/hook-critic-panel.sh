...
case "$1" in
  ...
  panel)
    ...
    jq -n --arg r "$roster" --arg l "$learn" '{decision:"block", reason:("CRITIC PANEL: this turn changed code. Read the roster at " + $r + " and follow it (dispatch, token, and synthesis rules all live there)." + $l)}'
    ;;
esac
exit 0
